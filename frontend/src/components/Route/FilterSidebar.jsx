...
const FilterSidebar = ({ filters, onFilterChange, onApply, onReset }) => {
    ...
    const selectedCategories = filters.selectedCategory || [];
        ...
        <aside className="lg:col-span-1 flex flex-col space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm">
                ...
                <Dropdown
                    ...
                    onChange={(e) => onFilterChange('sortOption', e.value)}
                    options={sortOptions}
                    optionLabel="name"
                            ...
                            <div className="flex items-center">
                                <Checkbox
                                    ...
                                    onChange={(e) => {
                                        ...
                                        onFilterChange('selectedCategory', selected);
                                    }}
                                    checked={selectedCategories.includes(cat.name)}
                                />
                                <label htmlFor={cat.key} className="ml-2 text-gray-700">
                                    {cat.name}
                                </label>
                            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm">
                <h3 className="text-lg font-semibold text-gray-800 flex items-center mb-4">
                    <i className="pi pi-clock mr-2"></i>Süre Filtreleri
                </h3>
                <div className="space-y-4">
                    {durations.map((dur) => (
                        <div key={dur.key} className="flex items-center justify-between">
                            <div className="flex items-center">
                                <RadioButton
                                    inputId={dur.key}
                                    name="duration"
                                    value={dur.key}
                                    onChange={(e) => onFilterChange('selectedDuration', e.value)}
                                    checked={filters.selectedDuration === dur.key}
                                />
                                <label htmlFor={dur.key} className="ml-2 text-gray-700">
                                    {dur.name}
                                </label>
                            </div>
                            <span className="text-sm bg-gray-100 text-gray-600 font-medium px-2 py-0.5 rounded-full">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm">
                <h3 className="text-lg font-semibold text-gray-800 flex items-center mb-3">
                    <i className="pi pi-map-marker mr-2"></i>Mesafe (km)
                </h3>
                <p className="text-sm text-gray-600 mb-3">Maksimum Mesafe: {filters.distanceValue} km</p>
                <Slider
                    value={filters.distanceValue}
                    onChange={(e) => onFilterChange('distanceValue', e.value)}
                    min={1}
                    max={50}
                    className="w-full"
                />
            ...
            <div className="space-y-3">
                ...
                <button
                    onClick={onReset}
                    className="w-full flex justify-center items-center py-3 px-4 font-semibold text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-100"
                >
